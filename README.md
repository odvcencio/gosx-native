...
- **Auth and session plumbing:** native runtimes now include in-memory token stores, secure platform token stores, refreshable token-store contracts, bearer-auth transport wrappers with 401 refresh/retry, HMAC request-signing transports, generated client constructors that accept token stores, per-endpoint auth policy metadata, generated route auth metadata, and route guards; higher-level auth templates and session-exchange flows are still open.
...
- **Dev workflow:** `gsxnative dev` now watches GoSX source/config files and regenerates native sources through the build pipeline; `--build` lets the loop invoke Xcode/Gradle and preserves forwarded simulator/Gradle build flags. Hot reload triggers, server proxying, install/launch orchestration, and richer device targeting still need production integration.
- **Release packaging:** Android release tasks, iOS release archive/export defaults, and deterministic JSON artifact manifests are available through `gsxnative build --release`; signing, flavors, environment config, and artifact publishing are not production templates yet.
