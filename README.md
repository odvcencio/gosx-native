...
- **Auth and session plumbing:** native runtimes now include in-memory token stores, secure platform token stores, mutable/refreshable token-store contracts, bearer-auth transport wrappers with 401 refresh/retry, password/OAuth/WebAuthn/custom auth exchange strategies for `/api/auth/exchange`, auth session helpers that persist access tokens, scaffolded native auth helpers backed by Keychain/Keystore stores, HMAC request-signing transports, generated client constructors that accept token stores, per-endpoint auth policy metadata, generated route auth metadata, and route guards. Remaining work: provider SDK/browser challenge handoff and production IdP-specific adapters.
...
- **Operational hardening:** named in-memory caching, validation failures, retry counts, exponential retry backoff controls, telemetry-safe structured diagnostics, offline/network policy runtime APIs with platform status providers, provider-neutral crash reporting hooks, crash reporter fanout, and crash attribute redaction are present; provider-specific crash SDK upload adapters still need production integration.
