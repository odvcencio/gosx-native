...
- **Auth and session plumbing:** native runtimes now include in-memory token stores, bearer-auth transport wrappers, generated client constructors that accept token stores, and per-endpoint auth policy metadata; secure storage, refresh, request signing, and route guards are still open.
- **Native bridge and escape hatches:** generated bridge specs, typed Swift/Kotlin bridge callers, capability manifests, and runtime capability checking are present. Remaining work: device/server capability negotiation, generated native-module skeletons, secure local capability implementations, and stricter `//gosx:native <target>` implementation checks.
