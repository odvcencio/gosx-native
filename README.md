...
**Status: Android and iOS counter vertical slices compile. The iOS demo builds and passes a Simulator UI smoke test; the Android demo regenerates Compose source and assembles a debug APK in CI. GoSX `.gsx` Counter, Panel, Greeter, Derived, Toggle, Profile, Roster, FormControls, Expressions, and Scene3D fixtures now lower through the shared NIR and emit deterministic SwiftUI/Compose source. Scene3D lowers into a typed NIR payload, maps into the canonical `gosx/scene.IR` conformance contract, renders static meshes/models/points, instanced mesh batches, compute-particle placeholders, simple native HTML overlay text, and map-backed Scene3D spread props through runtime views, preserves post-fx declarations in generated native scene source, and CI regenerates, diffs, compiles, hosts, and UI-asserts checked-in Scene3D demo sources for both native app shells. No currently enumerated Scene3D native target tags fail validation, but visual post-fx passes, real GPU compute, renderer-backed DOM/WebView HTML overlays, and cross-target render conformance still require the durable renderer/backend layer.**
