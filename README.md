...
- **Typed data/action codecs:** generated clients now support primitive and named custom-model schemas, JSON input/output models, validation failures, named cache TTLs, action invalidation metadata, retry counts, and optimistic metadata. Remaining work: portable optimistic-update closures, route-bound resource state, richer collection schemas, and semantic lowering into NIR instead of directive scanning.
...
- **Dev workflow:** `gsxnative dev` now watches GoSX source/config files and regenerates native sources through the build pipeline; `--build` lets the loop invoke Xcode/Gradle and preserves forwarded simulator/Gradle build flags, including validated `--server-url` metadata for local server-backed clients. Hot reload triggers, install/launch orchestration, and richer device targeting still need production integration.
- **Release packaging:** Android release tasks, Android flavor-aware default tasks/artifact paths, iOS release archive/export defaults, Xcode/Gradle environment forwarding, env-backed release signing config, generated Android Gradle signing hooks, deterministic local artifact publishing, and redacted JSON artifact manifests are available through `gsxnative build --release`; remote store publishing is not a production template yet.
