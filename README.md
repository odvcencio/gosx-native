...
- **Auth and session plumbing:** native runtimes now include in-memory token stores, secure platform token stores, refreshable token-store contracts, bearer-auth transport wrappers with 401 refresh/retry, generated client constructors that accept token stores, and per-endpoint auth policy metadata; request signing and route guards are still open.
