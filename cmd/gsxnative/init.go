...
- Watch and regenerate native sources: `+"`gsxnative dev all`"+`
- Watch, regenerate, and invoke native tools: `+"`gsxnative dev ios --build --simulator \"iPhone 16\"`"+`
- Trigger external reload tooling after each successful dev cycle: `+"`gsxnative dev all --reload-stamp .gsxnative/reload.json --reload-command './scripts/reload-native.sh'`"+`
- List Android/iOS launch targets: `+"`gsxnative devices --json`"+`
- Build, install, and launch Android on a selected emulator/device: `+"`gsxnative dev android --launch --device emulator-5554`"+`
- Build, install, and launch iOS on a booted Simulator: `+"`gsxnative dev ios --launch --ios-bundle-id com.example.%s`"+`
- Write release artifact metadata: `+"`gsxnative build all --release --artifact-manifest build/gsxnative-artifacts.json`"+`
...
- Optional release signing config: `+"`.gsxnative/signing.json`"+`
...
func initSigningGitignore() string {
	return `signing.json
*.keystore
*.jks
`
